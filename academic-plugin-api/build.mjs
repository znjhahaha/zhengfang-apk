...
writeFileSync(new URL('./api-lock.json',import.meta.url),JSON.stringify({version:'1.0.0',apiVersion:1,sha256:Object.fromEntries(files.map(file=>[file,createHash('sha256').update(readFileSync(new URL(file,import.meta.url))).digest('hex')]))},null,2)+'\n');
