...
import {nativeDefinitions,nativeManifestFields} from './native-schema.mjs';
...
export const methods = {
 ...
 'service.page':'ServicePage','service.action':'ServiceReceipt','ui.init':'UiResult','ui.reduce':'UiResult','task.run':'TaskResult','data.query':'DataResult'
};
...
Object.assign($defs,nativeDefinitions);
...
export const manifest = {$schema:contract.$schema,$id:'https://zfplugin.local/api/3/manifest.schema.json',...obj({
 ...
 kind:string({enum:['configuration','extension','independent','service','native']}),extends:string({enum:builtins}),entry:{const:'index.js'},
 capabilities:{...arr(string({enum:Object.keys(methods)}),30),uniqueItems:true},
 ...nativeManifestFields,
...
},['id','name','version','apiVersion','kind','capabilities','network','files'])};
