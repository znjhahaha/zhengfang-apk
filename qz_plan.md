...
- Scope: website-style `full_chain` task creation, filter semantics, task status, dry-run discovery, and live API execution.
- Non-goal: captcha solving and conflict-confirm automation.
...
- StrongZhi adapter stages:
  ...
  4. parse category configuration from HTML
  5. fetch list rows from category APIs
  6. match by website filters
  7. submit `*Oper` for live execution when `dry_run=false`
  8. return `dry_run_blocked` only when `dry_run=true`
...
- `dry_run=true` remains discovery-only.
- `dry_run=false` enters the live API execution path.
